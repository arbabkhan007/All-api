 ...
 *   4. Reload the spreadsheet. Use the "Hearth & Timber" menu.
 *
 * Brand
...
function showAbout() {
  var html = HtmlService.createHtmlOutput(
    '<div style="font-family:Georgia,serif;color:#202522;background:#F8F6F1;padding:16px">' +
      '<h2 style="color:#16352F;margin:0 0 8px">Hearth &amp; Timber</h2>' +
...
function emailMaintenanceReminders() {
  ...
  MailApp.sendEmail({
    to: email,
    subject: 'Hearth & Timber — maintenance due',
    htmlBody:
...
function emailUnreadDigest() {
  ...
  MailApp.sendEmail({
    to: email,
    subject: 'Hearth & Timber — unread project messages',
    htmlBody:
      '<div style="font-family:Georgia,serif;color:#202522">' +
...
function toast_(msg) {
  SpreadsheetApp.getActive().toast(msg, 'Hearth & Timber', 6);
}
